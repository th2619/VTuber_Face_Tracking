<script>
	...
	onMount(() => {
        ...
        ws.on('eventFromServer', function (message) {
	...
	function connectCamera() {
		...
		if (!ws || ws.readyState === WebSocket.CLOSED) {
			...
			ws.onopen = function () {
				console.log('Connected to the WebSocket server!');
				ws.send(JSON.stringify({ status: 'camera_connected' }));
			};
		}
	}
...
<div class="flex flex-col h-screen">
	<div class="text-center bg-gray-800 text-white py-4 text-2xl font-bold">VTuber Face Tracking</div>
	...
	<div class="flex flex-grow">
		<div class="w-1/2 bg-gray-100 p-4 flex">
			<video bind:this={videoElement} autoplay class="w-full h-full" />
		</div>
		<div class="w-1/2 bg-gray-200 p-4 flex items-center justify-center">
			<iframe src="/WebGL/index.html" frameborder="0" class="w-full h-full object-cover" />
		</div>
	</div>
</div>
